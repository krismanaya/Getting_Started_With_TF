...
# pprint(data[0:,0].shape) ## X 
# pprint(data[0:,1].shape) ## Y
...
x_m = tf.multiply(X, w) # X*m (number of fire) * weight
Y_predicted = tf.add(x_m,b) # (X*m + b) + plus the bias. 

# Step 5: use the square error as the loss function
# name your variable loss
distance = tf.subtract(Y,Y_predicted)
loss = tf.square(distance, name = "loss")
